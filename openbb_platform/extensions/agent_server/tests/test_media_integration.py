"""Real-I/O integration tests for :mod:`runtime tools._media`."""
...
def _free_port() -> int:
    """Bind to ``:0`` to get a kernel-allocated free port, then release."""
...
async def _payload_bytes(request: Any) -> bytes:
    """Return ``size`` random-ish bytes from a query param (default 1024)."""
    size = int(request.query_params.get("size", "1024"))
    # Deterministic content so tests can assert on byte values.
...
def _build_app() -> Starlette:
    ...
    async def png(_: Any) -> Response:
        # A 1x1 transparent PNG — real header so MIME detection works.
        body = bytes.fromhex(
    ...
    async def with_charset(_: Any) -> Response:
        # MIME with parameters — fetch_url must strip the charset suffix.
    ...
    async def no_ct_asgi(scope: Any, receive: Any, send: Any) -> None:
        """Bare ASGI app — bypasses Starlette's auto-injected content-type."""
        if scope["type"] != "http":
            return
...
class _ServerHandle:
    """A running uvicorn server pinned to a known port."""
...
async def test_fetch_url_strips_content_type_parameters(
    ...
    """``Content-Type: text/plain; charset=utf-8`` must become ``text/plain`` —"""
    fetched = await fetch_url(
...
async def test_fetch_url_raises_too_large_on_oversized_payload(
    ...
    """A 4 KB response with a 1 KB max should trip the streaming abort."""
...
async def test_fetch_url_raises_fetch_error_on_connection_refused() -> None:
    """Hitting an unbound port surfaces as a typed ``MediaFetchError``"""
    port = _free_port()  # bind+release to get a vacant port
    with pytest.raises(MediaFetchError):
        await fetch_url(
...
def test_flatten_handles_typed_block_chunks() -> None:
    ...
    # Only the two text blocks contribute; the image_url block is dropped.
...
def _synthesize_wav(duration_s: float, freq_hz: float = 440.0) -> bytes:
    """Generate a mono 24 kHz PCM-16 WAV of the requested duration."""
    sample_rate = 24000
...
async def test_probe_audio_duration_returns_close_to_expected() -> None:
    ...
    # ffprobe rounds slightly; require <50 ms drift.
...
async def test_split_audio_bytes_passes_short_clip_through_unchanged() -> None:
    ...
    # Short clip → single segment, no ffmpeg re-encode → raw bytes preserved.
    assert seg.data == wav
...
async def test_split_audio_bytes_chunks_long_clip_into_segments() -> None:
    ...
    # Each segment was actually transcoded (Opus-in-Ogg → magic header).
    for seg in segments:
...
def test_ffmpeg_required_helpers_raise_when_binary_missing(
    ...
    """When ``shutil.which`` returns ``None``, the helpers raise the typed"""
...
async def test_probe_audio_duration_raises_when_ffprobe_returns_non_numeric(
    ...
    """A non-numeric stdout from ffprobe surfaces as ``MediaError``."""
    from openbb_agent_server.plugins.tools._media import (
        MediaError,
        probe_audio_duration,
    )
...
async def test_split_audio_bytes_raises_when_ffmpeg_segment_fails(
    ...
    """Non-zero ffmpeg returncode mid-split surfaces as ``MediaError``."""
    from openbb_agent_server.plugins.tools._media import (
        MediaError,
        split_audio_bytes,
    )

    # First call is the probe (returns a duration that forces splitting);
    # the second is the segment ffmpeg call (fails).
    calls: list[str] = []
