...
| [`fetch_url`](fetch_url.md) | `fetch_url(url)` — SSRF-guarded fetch of one web page's readable text. Gated by the dedicated `fetch-url` Workspace toggle. Blocks private / loopback / link-local / cloud-metadata hosts; re-validates every redirect hop. Fetched text is untrusted DATA. |
