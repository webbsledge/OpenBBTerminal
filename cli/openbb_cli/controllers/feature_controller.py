        ...
        if column_names:
            for col in column_names:
                ...
                for next_col in column_names:
                    if next_col != col:
                        column_recursive[col][next_col] = {"--help": None, "-h": "--help"}
...
"""
    ...
    def call_info(self, other_args: list[str]):
        ...
        if ns_parser:
            if self.current_table is None:
                session.console.print("[yellow]No table selected. Use 'select' command first.[/yellow]")
                return
            ...
            session.console.print(f"Shape: {df.shape[0]} rows × {df.shape[1]} columns\n")
    ...
    def call_query(self, other_args: list[str]):
        ...
        if ns_parser:
            ...
            if hasattr(result, "model_dump"):
                data_obj = result.model_dump().get("results")
            else:
                data_obj = result
                ...
                if simple_filter:
                    ...
                    if value.startswith(("'", '"')):
                        ...
                        try:
                            ...
                            sql_value = str(eval(value))
                            # Quote if it's a string result
                            if not sql_value.replace(".", "").replace("-", "").isdigit():
                    ...
                    try:
                        ...
                        if ns_parser.save:
                            ...
                            session.console.print("[green]Table updated with result.[/green]")
                        else:
                            session.output_adapter.display(
                                data=eval_result,
            ...
            try:
                ...
                eval_result = eval(query_str, namespace)
                
                # Handle different output types
                if isinstance(eval_result, pd.DataFrame):
                    # Check if it's a filtered DataFrame (same columns as original)
                    if set(eval_result.columns) == set(df.columns) and len(eval_result) != len(df):
                        session.console.print(
                ...
                elif hasattr(eval_result, "__iter__") and not isinstance(eval_result, str):
    ...
    def call_modifycol(self, other_args: list[str]):
        ...
        if ns_parser:
            ...
            try:
                ...
                if "." in expr_str and any(method in expr_str for method in ["astype", "str", "dt", "fillna", "replace"]):
                    # For pandas methods, evaluate on the column itself
                    df[ns_parser.name] = eval(f"df['{ns_parser.name}'].{expr_str}", {"df": df})
                else:
    ...
    def call_join(self, other_args: list[str]):
        ...
        if ns_parser:
            ...
            if table_idx is None:
                session.console.print(f"[red]Table '{ns_parser.table}' not found in cache.[/red]")
                return

            if table_idx not in session.obbject_registry.all:
                session.console.print(f"[red]Table '{table_idx}' not found in cache.[/red]")
                return
            ...
            try:
                if ns_parser.on:
                    ...
                    merged_df = pd.merge(df_left, df_right, on=on_cols, how=ns_parser.how)
                ...
                if ns_parser.save:
                    ...
                    session.console.print("[green]Table updated with join result.[/green]")
                else:
                    session.output_adapter.display(
    ...
    def call_copy(self, other_args: list[str]):
        ...
        if ns_parser:
            ...
            if session.obbject_registry.register(new_result):
                ...
                session.console.print(
                    "[red]Failed to register copied table.[/red]"
                )
    ...
    def call_save(self, other_args: list[str]):
        ...
        if ns_parser:
            ...
            file_path = Path(session.user.preferences.data_directory) / ns_parser.filename
            
            try:
                if ns_parser.filename.endswith(".csv"):
                    df.to_csv(file_path, index=ns_parser.index)
                    session.console.print(
                        f"[green]Saved table to {file_path}[/green]"
                    )
                    ...
                    sheet_name = ns_parser.sheet_name if ns_parser.sheet_name else "Sheet1"
                    save_to_excel(
                        df=df,
                    ...
                    try:
                        ...
                        if table_exists and ns_parser.mode == "append":
                            ...
                            session.console.print(
                                f"[green]Saved table '{table_name}' ({len(df)} rows) to existing database {file_path}[/green]"
                            )
                        else:
                            session.console.print(
                                f"[green]Created new database and saved table '{table_name}' ({len(df)} rows) to {file_path}[/green]"
                            )
                    finally:
                        conn.close()
                else:
    ...
    def call_delete(self, other_args: list[str]):
        ...
        if ns_parser:
            ...
            if table_idx is None:
                session.console.print(f"[red]Table '{ns_parser.index}' not found in cache.[/red]")
                return
            ...
            if table_idx in session.obbject_registry.all:
                ...
                session.console.print(
                    f"[green]Deleted table '{table_idx}'[/green]"
                )
